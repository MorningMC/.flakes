{ config, lib, pkgs, ... }: {
	...
	home-manager.users.morningmc.programs.hyprlock.settings = {
		...
		in
		{
			...
			reload_time = 10; # Reload on every 5 seconds
		...
		label = let
			...
			getMediaMetadata = updateInterval: key: let
				# Construct the command to fetch metadata
			...
			in
			"cmd[update:${toString updateInterval}:true] ${command}"; # Allow updates triggered by SIGUSR2
			...
			mkControlButton = xOffset: text: onClickSubcommand: {
				...
				# Register the command to execute when the widget is clicked
				onclick = "${lib.getExe pkgs.playerctl} ${onClickSubcommand} && pkill -SIGUSR2 hyprlock"; # Trigger an update on each click
			};
		in
		[
			# Display media title
			{
				...
				# Declare text style
				text = getMediaMetadata 5000 "\\{{ title }}"; # Get media title every 5 seconds
				color = "$text_color";
				font_family = "$font";
				font_size = 14;
			}
			...
			# Display media artist
			{
				...
				# Declare text style
				text = getMediaMetadata 5000 "\\{{ artist }}"; # Get media artist every 5 seconds
			...
			# Display media album
			{
				...
				# Declare text style
				text = getMediaMetadata 5000 "\\{{ album }}"; # Get media album every 5 seconds
				color = "$secondary_text_color";
				font_family = "$font";
				font_size = 8;
			}
			...
			# Display media position & length
			{
				...
				text = getMediaMetadata 1000 "\\{{ duration(position) }} / \\{{ duration(mpris:length) }}";
				color = "$secondary_text_color";
				font_family = "$font";
				font_size = 10;
			}
